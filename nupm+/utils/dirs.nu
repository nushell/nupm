...
export def nupm-home-prompt [--no-confirm]: nothing -> bool {
  ...
  if ($env.NUPM_HOME | path exists) {
    if ($env.NUPM_HOME | path type) != dir {
      error make --unspanned {
        msg: (
          $"Root directory ($env.NUPM_HOME) exists, but is not a"
...
export def tmp-dir [subdir: path --ensure]: nothing -> path {
  let d = $DEFAULT_NUPM_TEMP
    | path join $subdir
    | path join (random chars --length 8)
